...
class ComputeEngine:
    ...
    def iterate(self):
        ...
        if (self._registration is None) or (self._registration_age() > 60 * 5):
            ...
            if task_hash is not None and task_data is not None and function_id is not None and kwargs is not None:
                ...
                if td is not None:
                    try:
                        ...
                        msg = {'type': 'statusUpdate', 'taskHash': task_hash, 'status': 'error', 'error': f'Unable to create job: {str(e)}'}
    ...
    def _renew_registration(self):
        ...
        registration = _http_json_post(f'{self._app_url}/api/register', {
            ...
            'computeEngineConfigUrl': f'{google_bucket_base_url}/{config_object_name}',
            'secret': self._secret
        })
        print(f'')
        print(f'==========================================================================================')
