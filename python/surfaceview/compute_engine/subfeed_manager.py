...
class Subfeed:
    ...
    def report_new_messages(self, position: int, messages: List[Any]):
        for i in range(len(messages)):
            ...
            print('Uploading to google cloud', self._google_bucket_name, object_name, messages[i])
            _upload_to_google_cloud(self._google_bucket_name, object_name, json.dumps(messages[i]).encode('utf-8'), replace=False)

        msg = {'type': 'subfeedUpdate', 'feedId': self._feed_id, 'subfeedHash': self._subfeed_hash, 'messageCount': position + len(messages)}
...
class SubfeedManager:
    ...
    def iterate(self):
        ...
        ret = kp.watch_for_new_messages(subfeed_watches, wait_msec=1000, signed=True)
        for k, v in self._subfeeds.items():
            if k in ret:
                new_messages = ret[k]
